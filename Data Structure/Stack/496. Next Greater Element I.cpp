...
public:
    std::vector<int> nextGreaterElement(std::vector<int>& nums1,
        ...
        std::unordered_map<int, int>
            umap;  // Store the relationship between nums2 and comp_res;

        // Fill umap.
        for (int i = nums2.size() - 1; i >= 0; --i) {
            while (!st.empty() && st.top() <= nums2[i]) {
                st.pop();  // Pop elements smaller than current element.
            }
            umap[nums2[i]] =
                st.empty() ? -1
                           : st.top();  // If stack is empty, no element behind
                                        // is bigger than current element.
