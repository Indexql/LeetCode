/*
   ...
   0 if there is no intersected node. listA - The first linked list. listB - The
   ...
   2 Output: No intersection Explanation: From the head of A, it reads as
   [2,6,4]. From the head of B, it reads as [1,5]. Since the two lists do not
   intersect, intersectVal must be 0, while skipA and skipB can be arbitrary
   values. Explanation: The two lists do not intersect, so return null.
...
public:
    ListNode* getIntersectionNode(ListNode* headA, ListNode* headB) {
        ...
        // ptrA & ptrB both point to nullptr if two lists do not intersect.
        return ptrA;
    }
};
